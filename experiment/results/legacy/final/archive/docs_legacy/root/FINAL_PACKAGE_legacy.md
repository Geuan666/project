...
## 1.1 2026-03-18 Paper-Facing Update

这次补充更新不重开 circuit localization 或 correctness，而是把两个剩余问题补成论文主图级叙事：

1. 最小 cue 如何沿 `L2H14 -> MLP11 -> MLP16 -> MLP19 -> L20H5 -> L21H12 -> L24H6 -> MLP27` 被读取、传播、放大并写成 `<tool_call>`。
2. `L16H4 -> MLP17 -> L23H6` 如何实现真正的 suppressive no-tool mechanism。

新的正式入口是：

- `data/paper_facing_main_report.md`
- `data/paper_facing_focused_evidence_table.csv`
- `data/paper_facing_claim_tiers.json`
- `data/paper_facing_still_unsolved.csv`
- `figures/PAPER_FIGURE_PLAN.md`
- `figures/figure_18_earliest_reader_vs_mlp11.png` 到 `figures/figure_25_suppression_stagewise_trajectory.png`

这些新增 artifact 不改变已有 signed circuit / sufficiency / necessity 结论，只把 Q1 和 Q2 的机制故事补成可直接进正文的图和主张边界。
...
### 5.5 Delivery-Object 2x2 与 Direction-Level 因果闭环

- 这组最终补证解决的是前面一直最薄弱的点：
  `L2H14` 到底是不是最早的 delivery-semantics writer，
  以及 `MLP11` 是否真的把 file-vs-answer 这条对象轴写进 shared scaffold。
- 先看干净的 `delivery_object_2x2_strict` 全量结果。
  行为上，`write_file` 与 `write_answer` 已经明显分开：
  decision score `1.316` 对 `0.092`，
  `tool_top1` `0.540` 对 `0.239`，
  `no_tool_top1` `0.456` 对 `0.749`。
  但 `L2H14` 仍是 frame-dominant：
  `lead_k` same-object cross-frame `0.897`，same-frame cross-object `1.000`；
  `z` same-object `0.953`，same-frame `0.966`。
  `MLP11` 则开始真正按 delivery object 分组：
  `mlp11_out` same-object cross-frame `0.992`，same-frame cross-object `0.974`，gap `+0.018`，object-wins `1.000`。
- whole-node patch 也开始把这件事写硬：
  `write` frame 下，`MLP11` 的 file-rescue `0.332`，
  object-decision `0.141`，
  boundary `0.604`，
  tool-top1 `0.718`；
  对比 `L2H14` 同一条件下只有 file-rescue `-0.019`、boundary `0.009`。
  所以在 node 级别，`MLP11` 已经明显比 `L2H14` 更像 first stable delivery-object writer。
- 但最终把机制闭环的是新的 direction-level causal intervention。
  这里不再 patch 整个 node，而是只编辑跨 frame 共享的 `file-vs-answer` direction。
  全量 `1722` 样本上：
  `L2H14` 的 shared object direction 虽然存在（write-vs-develop cosine `0.972`），但编辑它几乎不改变末端行为：
  `write_answer` 注入 file component 时 object-score 只变 `+0.0022`，tool logit `0.0`，`MLP27` 投影只变 `+1.44`。
  相比之下，只编辑 `MLP11` 的同一条 shared direction：
  `write_answer` 注入时 object-score `+0.678`，tool logit `+1.375`，distractor `-1.625`；
  从 `write_file` 擦除时 object-score `-0.189`，tool logit `-0.500`，distractor `+0.375`。
- 这条 `MLP11` shared direction 会被 `MLP16 -> MLP19` 明显放大，并进入晚层 writer。
  `write_answer` 上只改 `MLP11` direction 时，
  `MLP16 +9.26`，
  `MLP19 +20.26`，
  `L20H5 +1.80`，
  `L21H12 +8.26`，
  `L24H6 +27.38`，
  `MLP27 +94.68`。
  在 `develop` frame 上同样成立，只是 endpoint 行为更弱：
  `MLP16 +8.89`，
  `MLP19 +19.84`，
  `L21H12 +4.75`，
  `L24H6 +25.57`，
  `MLP27 +56.22`。
- 所以当前最强、并且已经可以论文级强写的闭环是：
  `最小 cue -> L2H14 -> MLP11 -> MLP16 -> MLP19 -> L20H5 -> L21H12 -> L24H6 -> MLP27`。
  更准确地说：
  `L2H14` 是 earliest head-level reader；
  它读取 opening-side bundle，其中已经带有弱的 file-vs-answer component；
  但 `MLP11` 才是 first stable delivery-object writer，
  而且只编辑 `MLP11` 的共享对象方向就足以把这条状态推进到 late writer。
- 详细 artifact 见：
  `data/delivery_object_2x2_strict_report.md`、
  `data/delivery_object_2x2_strict_patch_summary.csv`、
  `data/delivery_object_direction_report.md`、
  `data/delivery_object_direction_alignment_summary.csv`、
  `data/delivery_object_direction_intervention_summary.csv`，
  以及对应 per-sample CSV。
