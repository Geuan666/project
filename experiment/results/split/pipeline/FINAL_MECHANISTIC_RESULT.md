...
The resulting signed circuit remains highly faithful: full-circuit KL recovery `1.000` / `0.998`, top-1 `0.998` / `0.997`.
...
1. The model reads an instruction-level commitment cue from the first user instruction line. Swapping only that line is enough to flip the first-token decision across the whole dataset.
   Evidence: `clean_full` decision `3.203` and tool-top1 `1.000`, but `clean_with_corrupt_instruction` drops to decision `-4.594` and tool-top1 `0.000`; `corrupt_with_clean_instruction` flips back to tool-top1 `1.000`.
...
2. That commitment cue enters the tool-writing route through a user-conditioned late ingress path.
   ...
   Evidence: `L20H5` rescue `0.305`, `L21H1` `0.568`, `L21H12` `0.705`, `L21H12->MLP27` mediation `0.329`.
   Stepwise buildup: by `L20H5|L21H1|L21H12` the decision is already `0.693` with tool-top1 `0.695`; the full route reaches tool-top1 `0.939`.

3. `MLP27` writes this state into a `<tool_call>`-favoring output direction.
   Evidence: at steering alpha `1.5`, `corrupt_full` reaches tool-top1 `0.834`, `clean_schema_mismatch` reaches `0.959`, `clean_no_protocol` still reaches `0.587`.
   Backup search: on `corrupt_full`, MLP27 direct rescue is `0.807` while the best independent alternative under MLP27 block is only `0.369` from `L21H12`.
...
4. A competing no-tool chain pushes the same fixed tool environment back toward `no_tool`.
   ...
   Evidence: `MLP17` rescue `0.474`, `MLP17->L20H5` mediation `0.050`, full suppressive route no-tool top1 `0.517`.
