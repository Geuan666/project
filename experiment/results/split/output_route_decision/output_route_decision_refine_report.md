...
跨层几何也支持这点：`MLP11↔MLP16` cosine `0.040`，`MLP11↔MLP19` `0.051`，`MLP16↔MLP19` `-0.005`。三者几乎不共线，所以更合理的对象是“逐层重编码的连续状态”，而不是“固定向量搬运”。

进一步定义模块级对象：`R_module(x) = mean(r_MLP11(x), r_MLP16(x), r_MLP19(x))`。

## route score 的验证结果

- `MLP11`: clean `1.036`, corrupt `-1.016`, AUC `0.966`, clean>0 比例 `0.902`, corrupt<0 比例 `0.867`, 与最终 route margin 的 Spearman `0.773`。
- `MLP16`: clean `0.961`, corrupt `-1.000`, AUC `0.996`, clean>0 比例 `0.951`, corrupt<0 比例 `0.980`, 与最终 route margin 的 Spearman `0.855`。
- `MLP19`: clean `0.901`, corrupt `-1.032`, AUC `0.995`, clean>0 比例 `0.929`, corrupt<0 比例 `0.980`, 与最终 route margin 的 Spearman `0.865`。
- `R_module`: clean `1.015`, corrupt `-1.042`, AUC `0.995`, 与最终 route margin 的 Spearman `0.852`。

这些结果说明 clean/corrupt 不只在 logits 上分开，也在三个 anchor 的局部 route score 上稳定分开。

对 anchor 的 direction-only 干预结果：

- `MLP11`: patch-promote 后 route margin 变化 `1.591`，patch-erase 后 `-0.553`；inject 后本地 score 变化 `1.934`，`R_module` 变化 `0.776`，最终 route margin 变化 `1.390`；erase 后分别为 `-1.933` / `-0.775` / `-0.480`。
- `MLP16`: patch-promote 后 route margin 变化 `2.547`，patch-erase 后 `-1.529`；inject 后本地 score 变化 `1.899`，`R_module` 变化 `0.692`，最终 route margin 变化 `2.482`；erase 后分别为 `-1.899` / `-0.708` / `-1.416`。
- `MLP19`: patch-promote 后 route margin 变化 `2.511`，patch-erase 后 `-1.287`；inject 后本地 score 变化 `1.815`，`R_module` 变化 `0.605`，最终 route margin 变化 `2.518`；erase 后分别为 `-1.815` / `-0.605` / `-1.282`。
...
- `MLP11->MLP16`: promote route mediation `0.156`, erase route mediation `0.052`, promote target-score mediation `0.145`, erase target-score mediation `0.149`, 标签 `strong`。
- `MLP16->MLP19`: promote route mediation `0.095`, erase route mediation `0.059`, promote target-score mediation `0.105`, erase target-score mediation `0.129`, 标签 `strong`。
- `MLP16->MLP17`: promote route mediation `0.149`, erase route mediation `0.095`, promote target-score mediation `0.208`, erase target-score mediation `0.243`, 标签 `strong`。
- `MLP19->L20H5`: promote route mediation `0.044`, erase route mediation `0.034`, promote target-score mediation `0.261`, erase target-score mediation `0.355`, 标签 `weak`。
- `MLP19->L21H12`: promote route mediation `0.057`, erase route mediation `0.028`, promote target-score mediation `0.127`, erase target-score mediation `0.178`, 标签 `weak`。
- `MLP19->MLP27`: promote route mediation `0.145`, erase route mediation `0.059`, promote target-score mediation `0.280`, erase target-score mediation `0.205`, 标签 `weak`。
- `MLP19->L23H6`: promote route mediation `0.051`, erase route mediation `0.033`, promote target-score mediation `0.155`, erase target-score mediation `0.266`, 标签 `weak`。

与 group block 对照：

- `MLP19->tool_out_group`: promote group mediation `0.195`, erase group mediation `0.091`。
