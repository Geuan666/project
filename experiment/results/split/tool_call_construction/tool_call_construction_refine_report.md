...
- `<tool_call>` 的 logit margin 首次翻正仍在 `plus_L21H12`。
- `<tool_call>` 的 top-1 首次过半也仍在 `plus_L21H12`，但现在是实值 `0.859`，不再是旧 summary 里错误的 `1.0`。
...
- `corrupt_full`: route margin `-4.594`，logit margin `-5.125`，`<tool_call>` top1 `0.000`，boundary flip `0.000`，top token `I`。
- `plus_MLP19`: route margin `-1.931`，logit margin `-2.250`，`<tool_call>` top1 `0.043`，boundary flip `0.029`，top token `I`。
- `plus_L20H5`: route margin `-1.139`，logit margin `-1.375`，`<tool_call>` top1 `0.146`，boundary flip `0.125`，top token `I`。
- `plus_L21H1`: route margin `0.086`，logit margin `0.000`，`<tool_call>` top1 `0.489`，boundary flip `0.541`，top token `I`。
- `plus_L21H12`: route margin `1.394`，logit margin `1.500`，`<tool_call>` top1 `0.859`，boundary flip `0.922`，top token `<tool_call>`。
- `plus_L24H6`: route margin `1.723`，logit margin `1.875`，`<tool_call>` top1 `0.928`，boundary flip `0.980`，top token `<tool_call>`。
- `plus_MLP27`: route margin `2.291`，logit margin `2.500`，`<tool_call>` top1 `0.979`，boundary flip `0.999`，top token `<tool_call>`。

## L21H1 vs L21H12：限制性比较

- 固定 `MLP19 + L20H5` 之后，只加 `L21H1` 时，`<tool_call>` top1 为 `0.489`，logit margin 为 `0.000`。
- 固定 `MLP19 + L20H5` 之后，只加 `L21H12` 时，`<tool_call>` top1 为 `0.673`，logit margin 为 `0.625`。
- 加上 `L24H6` 后，`L21H1` 分支到 `0.732` / `0.750`，`L21H12` 分支到 `0.839` / `1.250`。
...
- `MLP19 + MLP27` 这条 shortcut node-set 的 `<tool_call>` top1 为 `0.686`，logit margin 为 `0.625`。
- `MLP19 + late_heads` 为 `0.928` / `1.875`；`late_heads + MLP27` 为 `0.939` / `2.125`；完整链 `full_chain` 为 `0.979` / `2.500`。
...
`MLP19` 先把上游 route state 扇出到 construction 区；`L20H5` 把这份状态绑定到文件名和函数体对象；随后 `L21H1` 与 `L21H12` 分化成两条 late routing，其中 `L21H1` 更偏 output-start/example，`L21H12` 更偏 tool-call example / instruction tail / protocol；`L24H6` 把已绑定状态压进调用起始格式；`MLP27` 最终把 `<tool_call>` 方向强写成首词偏好。`<tool_call>` 从 `L20H5` 开始被推向目标方向，在 `plus_L21H12` 首次过决策边界，在 `plus_L21H12` 首次过半，并在 `MLP27` 达到最强。

## 哪些说法可以强写

- `MLP27` 是主 writer。证据最强：clean direct margin `10.844`，平均 direct-logit lens rank=`1`。
- `L24H6` 更像 formatter / protocol commitment。证据最强：clean direct margin `2.255`，并且它把 stagewise top1 从 `0.859` 推到 `0.928`。
