...
```text
project/
├── CLAUDE.md                         # Claude Code 协作规则与项目总览
...
- 数据分离方式：按 `lang`（Python/Java/C++）× `clean_candidate`（15 种 clean 动词）分层随机抽样，`seed=42`，详见 `experiment/datasets/split_summary.json`。
- **train 集上跑的实验**结果放在 `experiment/results/split/` 对应子目录��。
