    ...
    def __init__(self, device):
        ...
        for century in range(11, 18):
            ...
            for year in range(century * 100 + 2, (century * 100) + 99):
                a = _TOKENIZER.encode(f" {year}", add_special_tokens=False)
                if a == [
                    _TOKENIZER.encode(f" {str(year)[:2]}", add_special_tokens=False)[0],
                    _TOKENIZER.encode(str(year)[2:], add_special_tokens=False)[0],
                ]:
        ...
        TOKENS = {
            i: _TOKENIZER.encode(f"{'0' if i<=9 else ''}{i}", add_special_tokens=False)[0] for i in range(0, 100)
        }
    ...
    for i in range(num_examples):
        ...
        prompts_tokenized.append(
            model.tokenizer.encode(
                prompts[-1], return_tensors="pt", add_special_tokens=False
            ).to(model.cfg.device)
        )
