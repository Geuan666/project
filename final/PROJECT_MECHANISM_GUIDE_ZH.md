...
我们的数据共 `1722` 对 `clean / corrupt` 样本，覆盖三种编程语言（Python 555 / Java 584 / C++ 583），15 种 clean 动词和 16 种 corrupt 动词。
...
为了排除"在 A 上找电路、又在 A 上测准确率"的过拟合风险，我们把全部 1722 对样本按 **语言 × clean 动词** 分层随机划分为两组（`seed=42`）：

| 集合 | 样本数 | 用途 |
| --- | ---: | --- |
| **train** | **1223** | 电路发现、方向计算、节点确定 |
| **test** | **499** | 用 train 集上学到的方向和电路结构做泛化验证 |

**以下所有章节中，主实验数字均来自 train 集；每个模块末尾附有 test 集上的验证数字。**

关键验证结论（详见第 9 节）：

- signed circuit 拓扑在 train 上重新发现后仍为 24 节点 / 64 边，与全量完全一致
- route score AUC：train 0.9946 → test **0.9943**
- construction 完整链 top1：train 97.9% → test **97.2%**
- suppression 完整链 no-tool top1：train 79.0% → test **78.2%**

所有核心指标在 test 上的下降不超过 1%。
...
`L11H5` 的 strongest handoff 目标不是更晚层，而就是 `MLP11` 本身（train 集，1223 样本）：

- source-only 时，`MLP11_local_rescue = 0.196`
- 同时只有 `MLP16_local_rescue = 0.053`
- `MLP19_local_rescue = 0.027`
...
在这个基础上，再看联合 `patch` 的结果（train 集）：

- 相对最佳单头，`MLP11_local_rescue` 中位增益 `+0.046`
...
这里的 `route score` 不是口头上的工作说法，而是一个有正式定义的节点局部对象。对任意锚点节点 `n`，记首个输出位置的节点激活为 `h_n(x)`，定义：

$$
d_n = \frac{\mu_n^{clean} - \mu_n^{corrupt}}{||\mu_n^{clean} - \mu_n^{corrupt}||},
\qquad
m_n = \frac{\mu_n^{clean} + \mu_n^{corrupt}}{2}
$$

$$
r_n(x) = \frac{\langle h_n(x) - m_n,\ d_n \rangle}{\langle \mu_n^{clean} - m_n,\ d_n \rangle}
$$
...
$$
R_{module}(x) = \text{mean}(r_{\text{MLP11}}(x),\ r_{\text{MLP16}}(x),\ r_{\text{MLP19}}(x))
$$

所以这里真正被模块 2 实现的，不是一条固定跨层向量，而是一类在每个锚点节点上都能计算、同号、可比较、可干预的一维路线对象。

**关键约束**：`d_n` 和 `m_n` 只在 train 集上计算。test 集上的 route score 使用的是 train 集的方向——这是验证泛化性的核心设计。
...
`MLP11`、`MLP16`、`MLP19` 三个节点上，`clean / corrupt` 的 `route score` 都稳定分开（train 集，1223 样本）：

| 节点 | clean 中位数 | corrupt 中位数 | AUC | Spearman |
| --- | ---: | ---: | ---: | ---: |
| `MLP11` | `1.036` | `-1.016` | `0.966` | `0.773` |
| `MLP16` | `0.962` | `-1.000` | `0.996` | `0.855` |
| `MLP19` | `0.901` | `-1.032` | `0.995` | `0.865` |
| **R_module** | **1.015** | **-1.042** | **0.995** | **0.852** |
...
更重要的是，这个 `route score` 不是纯描述量。沿各自的 local route direction 做 `inject / erase` 时，会同时看到三类量同步变化：

- 节点本地 `route score`
- 模块平均分数 `R_module`
- 最终输出边界，也就是 route margin

也就是说，这个对象不是我们事后投影出来的一个好看统计量，而是一个可干预、可验证、会真实推动下游分叉和最终输出变化的机制对象。
...
| --- | ---: | ---: |
| `MLP11 -> MLP16` | `0.156` | `0.052` |
| `MLP16 -> MLP19` | `0.095` | `0.059` |
| `MLP16 -> MLP17` | `0.149` | `0.095` |
...
在 test 集（499 样本）上，用 train 集的方向投影：

| 节点 | Train AUC | **Test AUC** | Train Spearman | **Test Spearman** |
| --- | ---: | ---: | ---: | ---: |
| `MLP11` | 0.966 | **0.968** | 0.773 | **0.762** |
| `MLP16` | 0.996 | **0.996** | 0.855 | **0.843** |
| `MLP19` | 0.995 | **0.993** | 0.865 | **0.856** |
| **R_module** | **0.995** | **0.994** | **0.852** | **0.841** |

AUC 在 test 上几乎不降（最大差 0.002），Spearman 下降在 0.01 量级。这说明 route score 不是对 train 集的过拟合，而是模型内部真实存在的稳定表示。

### 5.7 这一模块交给下一个模块什么

模块 2 会把状态分成两路：

- `MLP19` 作为 late fanout hub，把工具侧路线状态并行分发给模块 3 的多个接收节点，而不是只通过某一条单独主边把状态交出去
...
最关键的 `stagewise` 数字（train 集，1223 样本）：

| 阶段 | 路线分数 | `logit margin` | `<tool_call>` top1 |
| --- | ---: | ---: | ---: |
| `plus_MLP19` | `-1.931` | `-2.250` | `0.043` |
| `plus_L20H5` | `-1.139` | `-1.375` | `0.146` |
| `plus_L21H1` | `0.086` | `0.000` | `0.489` |
| `plus_L21H12` | `1.394` | `1.500` | `0.859` |
| `plus_L24H6` | `1.723` | `1.875` | `0.928` |
| `plus_MLP27` | `2.291` | `2.500` | `0.979` |

这说明：

- `L20H5` 之后，`<tool_call>` 还没真正过边界
- `L21H1` 之后，`<tool_call>` 首次接近过半
...
固定 `MLP19 + L20H5` 之后（train 集）：

- 只加 `L21H1`：`<tool_call>` top1 `0.489`
- 只加 `L21H12`：`<tool_call>` top1 `0.673`

再加上 `L24H6`：

- `L21H1` 分支：top1 `0.732`
- `L21H12` 分支：top1 `0.839`

这说明：

- `L21H1` 不是噪音，它确实能把 `<tool_call>` 推近边界
...
在 test 集上按同样的节点顺序做 stagewise（499 样本）：

| 阶段 | Train top1 | **Test top1** | Δ |
| --- | ---: | ---: | ---: |
| `+MLP19` | 0.043 | **0.048** | +0.5% |
| `+L20H5` | 0.146 | **0.160** | +1.4% |
| `+L21H1` | 0.489 | **0.537** | +4.8% |
| `+L21H12` | 0.859 | **0.862** | +0.3% |
| `+L24H6` | 0.928 | **0.912** | -1.6% |
| `+MLP27` | **0.979** | **0.972** | **-0.7%** |

最终阶段 train/test 差距仅 0.7%，逐步成形的轨迹形状也完全一致。
...
| --- | ---: | ---: | ---: | ---: |
| `L16H4` | `-0.250` | `+0.375` | `-0.657` | `0.010` |
| `L16H4 + MLP17` | `-1.000` | `+1.625` | `-2.414` | `0.299` |
| `L16H4 + MLP17 + L23H6` | `-1.625` | `+2.750` | `-3.951` | `0.790` |
...
在 test 集上按同样的节点做 suppression stagewise（499 样本）：

| 阶段 | Train no-tool top1 | **Test no-tool top1** | Δ |
| --- | ---: | ---: | ---: |
| `L16H4` | 0.010 | **0.014** | +0.4% |
| `+MLP17` | 0.299 | **0.271** | -2.8% |
| `+L23H6` | **0.790** | **0.782** | **-0.8%** |

最终阶段差距不到 1%，抑制链在未见数据上同样有效。
...
- 模块 1 把状态交给 `MLP11`
- 模块 2 把状态在 `MLP16` 分叉，并在 `MLP19` 以并行 fanout 的方式分发到工具侧
...
以下汇总全量、train、test 三套数字，证明电路发现和验证的分离没有引入过拟合。

### 9.1 电路拓扑

| 指标 | 全量 (1722) | Train (1223) |
| --- | ---: | ---: |
| 节点数 | 24 | 24 |
| 边数 | 64 | 64 |
| KL recovery (promote) | 1.000 | 1.000 |
| KL recovery (suppress) | 0.998 | 0.998 |

电路拓扑在 train 子集上完全复现。

### 9.2 Route Score

| 节点 | 全量 AUC | Train AUC | Test AUC |
| --- | ---: | ---: | ---: |
| MLP11 | 0.967 | 0.966 | **0.968** |
| MLP16 | 0.996 | 0.996 | **0.996** |
| MLP19 | 0.994 | 0.995 | **0.993** |
| **R_module** | **0.995** | **0.995** | **0.994** |

### 9.3 Construction Stagewise

| 阶段 | 全量 top1 | Train top1 | Test top1 |
| --- | ---: | ---: | ---: |
| +MLP19 | 0.045 | 0.043 | 0.048 |
| +L20H5 | 0.150 | 0.146 | 0.160 |
| +L21H1 | 0.503 | 0.489 | 0.537 |
| +L21H12 | 0.859 | 0.859 | 0.862 |
| +L24H6 | 0.923 | 0.928 | 0.912 |
| **+MLP27** | **0.977** | **0.979** | **0.972** |

### 9.4 Suppression Stagewise

| 阶段 | 全量 no-tool top1 | Train no-tool top1 | Test no-tool top1 |
| --- | ---: | ---: | ---: |
| L16H4 | 0.012 | 0.010 | 0.014 |
| +MLP17 | 0.289 | 0.299 | 0.271 |
| **+L23H6** | **0.783** | **0.790** | **0.782** |

### 9.5 小结

所有核心指标在 test 集上的变化：

- Route score AUC 最大降幅：**0.002**
- Construction 最终 top1 降幅：**0.7%**
- Suppression 最终 no-tool top1 降幅：**0.8%**

这说明我们发现的电路和 route score 方向不是统计巧合或过拟合产物，而是 Qwen3-1.7B 内部在处理工具调用决策时真实运行的计算模式。
...
| `route score` 在 test 集上泛化（AUC 降幅 < 0.002） | 可以强写 |
...
| 全部核心指标在 held-out test 上降幅 < 1% | 可以强写 |
...
> 这项工作的核心不是"模型里有一条抽象轴被放大"，而是"最小开头线索先改变输出路线状态；如果工具路线占优，模型就把 `<tool_call>` 组装出来；如果直接回答路线占优，另一条抑制链就会把 `no_tool` 写强并压住工具路线——而且这些结论在发现和验证使用不同数据的条件下依然成立"。
